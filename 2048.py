...
# Initialize the game state variables with the default board
mat = game_logic.start_game()  # Use the default board initialization
...
def key_press(key):
    ...
    if game_over:
        if key == 'r':
            mat = game_logic.start_game()  # Restart with the default board
            graph = game_logic.board_to_graph(mat)
            game_over = False
            score = 0
            draw_board()
...
# Draw the default board (initial state)
draw_board()
